...
import 'package:picture_in_picture_example/app/core/config/config.dart'
    as config;
...
class _VideoCallPageState
  ...
  _initEngine() async {
    widget.engine = await RtcEngine.create(config.appId);
    this._addListeners();

    await widget.engine.enableVideo();
    await widget.engine.startPreview();
    await widget.engine.setChannelProfile(ChannelProfile.LiveBroadcasting);
    await widget.engine.setClientRole(ClientRole.Broadcaster);
  }

  _addListeners() {
    widget.engine?.setEventHandler(RtcEngineEventHandler(
  ...
  _joinChannel() async {
    ...
    await widget.engine?.joinChannel(config.token, channelId, null, config.uid);
  }
                    ...
                    remoteUid.map(
                      (e) => GestureDetector(
                        ...
                        child: Container(
                          width: 120,
                          height: 120,
                          child: RtcRemoteView.SurfaceView(uid: e),
                        ),
                      ),
                    ),
                  ),
